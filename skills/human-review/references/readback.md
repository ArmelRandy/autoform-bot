...
as a review card: frontmatter copying `declaration` and `hash` from the packet
manifest entry you were given and naming the model that wrote the text, a
...
A read-back testifies about one skeleton. If the skeleton changes, the audit
reports the read-back as stale and it must be rewritten from the new packet.
