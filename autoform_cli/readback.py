...
hash it testifies about. When the
skeleton moves, the read-back is stale and the audit says so; the renderer
still shows it, marked as testimony about an earlier skeleton, rather than
silently presenting stale evidence as current.
"""
...
class Readback:
    ...
    def status(self, skeleton: DeclarationSkeleton) -> str:
        """``current`` when it testifies about this skeleton, else ``stale``."""

        return "current" if self.skeleton_hash == skeleton.hash else "stale"
    ...
    *,
    ...
    content = "\n".join(
        [
            ...
            f"# {declaration.name}",
            "",
            f"Article `{node_id}` · {declaration.kind} · skeleton `{declaration.hash}` · read back by {model}.",
            "",
            "## Skeleton",
            "",
            "```lean",
...
def readback_findings(
    ...
    for node in report.nodes:
        for declaration in node.declarations:
            ...
            elif readback.status(declaration) != "current":
                findings.append(
                    ReadbackFinding(
                        node.node_id,
                        declaration.name,
